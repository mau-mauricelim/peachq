/* ops/q_rand.c — the `?` roll / deal / permute / generate domain, drawing from
 * the ONE rng owner (core/rand.h, PCG64-DXSM) that the roll and guid kernels
 * also draw from, so `\S` reproduces every arm alike.
 *
...
#include "core/rand.h"     /* ray_rand_u64 / ray_rand_below — THE rng owner */
#include "lang/internal.h" /* ray_rand_fn, ray_guid_fn, RAY_IS_TEMPORAL64/F */
 ...
 * Every arm draws 64 bits at a time from the one stream (`\S n` re-seeds them
 * all); each right-operand form yields a result of y's type per the docs table. */
...
static ray_t* gen_syms(int64_t n, ray_t* ysym, int distinct) {
    ...
    for (int64_t i = 0; i < n; i++) {
        ...
        int64_t id;
        for (;;) {
            for (int64_t j = 0; j < m; j++) buf[j] = letters[ray_rand_u64() & 15];
 ...
 * 53 random bits give the fraction (every one exact in a double); a rounding
 * hit at the top is clamped back below y so the [0,y) contract holds exactly. */
static ray_t* gen_floats(int64_t n, ray_t* y) {
    ...
    out->len = n;
    for (int64_t i = 0; i < n; i++) {
        uint64_t u = ray_rand_u64() >> 11;
        double v = fy * ((double)u / 9007199254740992.0);       /* / 2^53 */
        if (f32) {
            float fv = (float)v;
...
static ray_t* gen_bits(int64_t n) {
    ...
    if (RAY_IS_ERR(out)) return out;
    out->len = n;
    for (int64_t i = 0; i < n; i++) ((bool*)ray_data(out))[i] = ray_rand_u64() & 1;
    return out;
}
...
static ray_t* gen_bytes(int64_t n) {
    ...
    out->len = n;
    for (int64_t i = 0; i < n; i++)
        ((uint8_t*)ray_data(out))[i] = (uint8_t)(ray_rand_u64() & 0xFF);
    return out;
}
...
static ray_t* gen_temporal(int64_t n, ray_t* y) {
    ...
    if (RAY_IS_TEMPORALF(tag)) {              /* datetime rides the float roll
        ...
        for (int64_t i = 0; i < n; i++) d[i] = m ? (int64_t)ray_rand_below((uint64_t)m) : 0;
    }
...
static ray_t* gen_chars(int64_t n) {
    ...
    for (int64_t i = 0; i < n; i++) b[i] = (char)('a' + ray_rand_below(26));
 ...
 * take the first n (kdb deal / permute).  The partner index MUST come from the
 * bounded draw: a raw `% range` biases every permutation, and on a 15-bit CRT
 * truncates the prefix into a 32K window.  n<=total required.  Result is an
 * owned I64 vector. */
static ray_t* deal_indices(int64_t n, int64_t total) {
    ...
    for (int64_t i = 0; i < n; i++) {
        ...
        int64_t j = i + (int64_t)ray_rand_below((uint64_t)range);
