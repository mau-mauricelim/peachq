...
Prebuilt `q` binaries are on the [latest release](https://github.com/peachq-org/peachq/releases/latest), in two
flavours per platform:

| | `peachq-<platform>.*` (standard) | `peachq-<platform>-duckdb.*` (with DuckDB) |
|---|---|---|
| Contents | `q` alone | `q` + the DuckDB library beside it + the `httpfs` and `aws` extensions |
| Use it for | q, IPC, CSV/JSON, the kdb on-disk format | the above plus `s)` SQL, Parquet, and `s3://` reads — offline, nothing to install |
| Linux binary | static musl: any distro, any x86-64 CPU | glibc-dynamic: glibc ≥ 2.28 (RHEL/Rocky 8+, Debian 10+, Ubuntu 20.04+) and a system `libstdc++.so.6` (GCC 6+); older or musl systems use the standard build |

Windows and macOS ship the same `q` in both archives; only Linux differs, because a static binary cannot load a
shared library. The `-duckdb` archive works with no network: the bundled extensions are pinned to the bundled
DuckDB version and `q` points DuckDB at its own `extensions/` directory, so `LOAD httpfs`, `LOAD aws` and an
`s3://` URL never reach for the internet to fetch code (your data still does). On macOS, clear the quarantine on
the whole unpacked directory (`xattr -dr com.apple.quarantine .`), not just `q`.
...
**With DuckDB** (any platform): download the `-duckdb` archive instead and unpack it whole — `q` finds
`libduckdb.so` / `libduckdb.dylib` / `duckdb.dll` and `extensions/` beside itself:

```bash
curl -fsSL https://github.com/peachq-org/peachq/releases/latest/download/peachq-linux-x86_64-duckdb.tar.gz | tar xz
./q
q)\l pq
q)s)SELECT version()
q)select from `:s3://my-bucket/trades.parquet    / after .duckdb.secret[...] or the AWS credential chain
```
